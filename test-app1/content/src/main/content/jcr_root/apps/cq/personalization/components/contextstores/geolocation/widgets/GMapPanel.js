if (typeof CQ !== 'undefined' && typeof CQ.Ext !== 'undefined') {
    CQ.Ext.namespace('CQ.Ext.ux.panel');

    CQ.Ext.ux.GMapPanel = CQ.Ext.extend(CQ.Ext.Panel, {
        /**
         * @cfg {Boolean} border
         * Defaults to <tt>false</tt>.  See {@link CQ.Ext.Panel}.<code>{@link CQ.Ext.Panel#border border}</code>.
         */
        border: false,
         ...
         * <pre><code>
         ...
         listeners: {
         click: function(e){
         CQ.Ext.Msg.alert('Its fine', 'and its art.');
         }
         }
         },{
         lat: 42.339419,
        ...
        initComponent : function() {
            ...
            CQ.Ext.ux.GMapPanel.superclass.initComponent.call(this);
            ...
            if (this.rendered) {
                ...
                (function() {
                    ...
                    if (typeof this.setCenter === 'object') {
                        ...
                        this.getMap().setOptions({'mapTypeControl': false});
                    }
        ...
        afterRender : function() {
            ...
            CQ.Ext.applyIf(this, wh);

            CQ.Ext.ux.GMapPanel.superclass.afterRender.call(this);
        ...
        onResize : function(w, h) {
            ...
            CQ.Ext.ux.GMapPanel.superclass.onResize.call(this, w, h);
        ...
        setSize : function(width, height, animate) {
            ...
            CQ.Ext.ux.GMapPanel.superclass.setSize.call(this, width, height, animate);

            // check for the existance of the google map in case setSize is called too early
            if (CQ.Ext.isObject(this.getMap())) {
                google.maps.event.trigger(this.getMap(), 'resize');
                if (this.lastCenter) {
                    this.getMap().setCenter(this.lastCenter, this.zoomLevel);
                }
        ...
        addMarker : function(point, marker, clear, center, listeners) {
            ...
            var mark = new google.maps.Marker(CQ.Ext.apply(marker, {
                position: point
            }));
        ...
        addPolyline : function(points, linestyle) {
            ...
            CQ.Ext.each(points, function(point) {
                plinepnts.push(new google.maps.LatLng(point.lat, point.lng));
            }, this);

            var pline = new google.maps.Polyline(CQ.Ext.apply({
                path: plinepnts
            }, linestyle));
        ...
        hideMarkers : function() {
            CQ.Ext.each(this.cache.marker, function(mrk) {
                mrk.setMap(null);
            });
        },
        // private
        ...
        addMapControls : function() {
            ...
            if (this.gmapType === 'map') {
                if (CQ.Ext.isArray(this.mapControls)) {
        ...
        addOptions : function() {
            ...
            if (CQ.Ext.isArray(this.mapConfOpts)) {
                var mc;
         ...
         * <pre><code>
         ...
         CQ.Ext.getCmp('my_map').geoCodeLookup(addr, undefined, false, true, undefined);
         }
         },{
         text: 'Zoom Fenway Park',
         handler: function(){
         CQ.Ext.getCmp('my_map').zoomLevel = 19;
         var addr = '4 Yawkey Way, Boston, MA, 02215-3409, USA';
         CQ.Ext.getCmp('my_map').geoCodeLookup(addr, undefined, false, true, undefined);
         }
         },{
         text: 'Low Accuracy',
         handler: function(){
         CQ.Ext.getCmp('my_map').geoCodeLookup('Paris, France', undefined, false, true, undefined);
         }
         },{
         ...
         *
         * @param {Object} point A google.maps.LatLng object
         */
        ...
        geoErrorMsg : function(title, msg) {
            if (this.displayGeoErrors) {
                CQ.Ext.MessageBox.alert(title, msg);
            }
        },
        // private
        respErrorMsg : function(code) {
            CQ.Ext.each(this.respErrors, function(obj) {
                if (code == obj.code) {
                    // a friendly alert could be added here.
                    // CQ.Ext.MessageBox.alert(this.respErrorTitle, obj.msg);
                }
            }, this);
        },
        // private
        getLocationTypeInfo: function(location_type, property) {
            var val = 0;
            CQ.Ext.each(this.locationTypes, function(itm) {
    ...
    CQ.Ext.reg('gmappanel', CQ.Ext.ux.GMapPanel);
