...
%><%@include file="/libs/foundation/global.jsp" %><%
                ...
                //TODO: adapt to offerpages?
                String imageUrl = TeaserUtils.getImage(myPage);
                values.put("image", (imageUrl == null ? "/libs/cq/personalization/templates/experience/thumbnail.png" : imageUrl) );
                ...
                if(content != null) {
                    try {
                        ...
                        Iterator<Resource> iter = parent.getResourceResolver().findResources(
                            "/jcr:root/content//*[(@campaignpath = '" + campaignPath + "') and " +
                        	"(@sling:resourceType = 'cq/personalization/components/mbox/start')]", 
                        	"xpath");
                        int i, length = 100;
                        ...
                        if(closest != null) {
                            ...
                            if(parent != null) {
                                Page offerContainingPage = parent.adaptTo(Page.class);
                                values.put("touchpointTitle", offerContainingPage.getTitle() + " (" + closest.getName() + ")");
                                values.put("touchpointChannel", (MobileUtil.isMobileResource(parent.getParent())? "Mobile": "Web"));
                                values.put("simulateUrl", offerContainingPage.getPath() + ".html");
                            }
                        }
